...
def worker(options, use_lxml):
    ...
    rc = Redis(options.redis_host, options.redis_port)
    ...
    def _put_url(url):
        if not rc.sismember(cns.PARSED_URLS_KEY, url):
            rc.sadd(cns.PARSED_URLS_KEY, url) # можно сократить вызовы, используя его в условии
            rc.lpush(cns.URL_QUEUE_KEY, url)
            logger.debug('Put new url {}'.format(url))

    while True:
        start_time = time.time()
        data_key = rc.rpop(cns.DATA_QUEUE_KEY)
        if not data_key:
            continue
        elif data_key.decode() == cns.FINISH_COMMAND:
            break
        else:
            try:
                data = rc.hget(data_key, 'content')
                rc.hdel(data_key, 'content')
                base_url = data_key.decode().replace(cns.DATA_KEY_PREFIX, '')
                documents = []
